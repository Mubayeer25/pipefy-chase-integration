...
CHASE_URL = config.get("Chase", "CHASE_URL_PROD")
CHASE_USERNAME = config.get("Chase", "CHASE_USERNAME_PROD")
CHASE_PASSWORD = config.get("Chase", "CHASE_PASSWORD_PROD")
...
AGENCY_PRODUCT_TABLE_MAP = {
    "2": "306759853",  # Levergy Marketing Agency (Pty) Ltd Products
    "8": "306759851",  # Dalmatian Communications (Pty) Ltd Products
    "9": "306759773",  # The Up and Up Group (Pty) Ltd Products
}
...
def build_pipefy_mutations(records, table_id, action, field_mapping):
    ...
    for i, record in enumerate(records):
        ...
        safe_product_id = re.sub(r'\W|^(?=\d)', '_', str(record.get('product_id', '')))
        safe_bu_id = re.sub(r'\W|^(?=\d)', '_', str(record.get('business_unit_id', '')))
        unique_suffix = f"{alias_action}_{i}_{safe_product_id}_{safe_bu_id}"
        pipefy_record_id = record.get("pipefy_record_id")

        if action == "create":
            if not all(str(record.get(k)) for k in key_identifiers if k in record):
                log(f"Skipping record creation for table {table_id}: Missing key identifier(s)")
                continue
            ...
            # CORRECTED: Use title as a separate parameter, not in fields_attributes
            ...
            fields_to_set = {
                ...
                if key not in key_identifiers and key in record
            }
            fields_to_set["status"] = sanitize_graphql_string(record.get("status", "Active"))

            if not fields_to_set:
                log(f"Skipping update for record {pipefy_record_id}, no changes.")
                continue

            # Build array for updateFieldsValues mutation
            field_values = []
            for pipefy_slug, val in fields_to_set.items():
            ...
            # ADDED: Check if title needs updating and add separate mutation
...
def execute_pipefy_mutations(mutations):
        ...
        if not response or "errors" in response:
            ...
            for alias, result in data.items():
                if result is None:
                    ...
                    if result.get("table_record") or result.get("clientMutationId") is not None:
                        success_count += 1
            
            log(f"Batch {i//batch_size + 1} finished. Success count: {success_count}/{len(batch)} (null responses: {null_count})")
    ...
    if all_successful:
        ...
        log("ERROR: One or more Pipefy mutation batches failed or had partial failures.")
                ...
                if isinstance(products, list):
                    ...
                    for product in products:
                        ...
                        prod_client_data = {
                            "config_id": config_id,
                            "client_id": client_id,
                            "client_name": client.get("CustomerName"),
                            "product_id": prod_id,
